...
arguments
    ...
    filterArguments.functionType (1,:) char {mustBeMember(filterArguments.functionType,...
...
switch lower(filterArguments.functionType)
    ...
    case 'gaussian'
        if isempty(filterArguments.fullWidthHalfMax)
            ...
            sigmaX = (filterArguments.fullWidthHalfMax / 100) * kMaxX;
            sigmaY = (filterArguments.fullWidthHalfMax / 100) * kMaxY;
        end
        xWeights = exp(-(kGridX.^2) / (2 * sigmaX^2));
        yWeights = exp(-(kGridY.^2) / (2 * sigmaY^2));
    case 'hamming'
...
if getFlags(MRSIStruct, 'spatialFT') == 0
    ...
    data = getData(MRSIStruct);
    kernel = ifftshift(ifft2(fftshift(weightMatrix)));
    kernel = kernel / sum(kernel(:)); 
    
    for iExtra = 1:getSizeFromDimensions(MRSIStruct, {'extras'})
        data(:,:,iExtra) = conv2(data(:,:,iExtra), kernel, 'same');
    end
end
