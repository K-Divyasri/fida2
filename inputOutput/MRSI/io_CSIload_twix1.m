...
function MRSIStruct = io_CSIload_twix1(filename, kFile)
    arguments
        filename (1,:) {mustBeFile}
        kFile string = ""
    end

    % If kFile is empty, fall back to op_CSIload_twix
    if kFile == ""
        MRSIStruct = io_CSIload_twix(filename);
        return;
    end
...
function dims = fillDimsField(sqzDims)
    ...
    for i = 1:length(sqzDims)
        key = sqzDims{i};
        if isKey(SiemenstoMRSI, key)
            label = SiemenstoMRSI(key);
            dims.(label) = i;
        else
            if dims.extras == 0
                dims.extras = i;
            else
                error('Multiple extra dimensions detected – revise fillDimsField.');
            end
        end
    end
end
